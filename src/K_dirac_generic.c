...
void block_d_plus_clover_PRECISION( vector_PRECISION eta, vector_PRECISION phi, int start, schwarz_PRECISION_struct *s, level_struct *l, struct Thread *threading ) {
  ...
  PRECISION* clover_vectorized = (PRECISION*) (s->op.clover_vectorized+start*6);
#else
  int k, *ind;
  ...
  if ( g.csw == 0.0 ) {
    ...
    for ( i=0; i<n; i++ ) {
#ifdef OPTIMIZED_SELF_COUPLING_PRECISION
      K_site_clover_PRECISION( (PRECISION*)(leta+12*i), (PRECISION*)(lphi+12*i), clover_vectorized+72*i );
#else
...
K_block_d_PRECISION(leta, lphi, start, s);
#else
